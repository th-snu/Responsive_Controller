...
class PPO(object):
    ...
    def initRun(self, pretrain, num_state, num_action, num_slaves=1):
        ...
        save_list = [v for v in tf.compat.v1.trainable_variables()
                     if v.name.find(name) != -1]
        self.saver = tf.compat.v1.train.Saver(var_list=save_list, max_to_keep=1)
    ...
    def initTrain(self, name, env, pretrain="", directory=None,
        ...
        if name is None:
            print("test_name is not specified, defaulting to 'test'")
            name = "test"
    ...
    def printInitialSetting(self):
        ...
        print_list.append("learning_rate : {}".format(
        ...
        print_list.append("steps_per_iteration : {}".format(
    ...
    def createActor(self):
        actor_trainer = tf.compat.v1.train.AdamOptimizer(
    ...
    def createCriticNetwork(self, name, input, TD, clip=True):
        ...
        with tf.compat.v1.variable_scope(name+'_Optimize'):
            value_loss = tf.reduce_mean(input_tensor=tf.square(critic.value - TD))
            loss = value_loss

        critic_trainer = tf.compat.v1.train.AdamOptimizer(
    ...
    def buildOptimize(self, name):
        try:
            self.state = tf.compat.v1.placeholder(
                tf.float32, shape=[None, self.num_state], name=name+'_state')
        ...
        with tf.compat.v1.variable_scope(name+'_Optimize'):
            self.action = tf.compat.v1.placeholder(
                tf.float32, shape=[None, self.num_action], name='action')
            self.TD = tf.compat.v1.placeholder(tf.float32, shape=[None], name='TD')
            self.GAE = tf.compat.v1.placeholder(tf.float32, shape=[None], name='GAE')
            self.old_neglogp = tf.compat.v1.placeholder(
                tf.float32, shape=[None], name='old_neglogp')
            ...
            clipped_ratio = tf.clip_by_value(
                self.ratio, 1.0 - self.epsilon, 1.0 + self.epsilon)

            surrogate = - \
                tf.reduce_mean(input_tensor=tf.minimum(
                    self.ratio*self.GAE, clipped_ratio*self.GAE))
            self.loss_actor = surrogate

        actor_trainer, self.actor_train_op = self.createActor()

        self.critic, self.critic_train_op, self.loss_critic = self.createCriticNetwork(
            name, self.state, self.TD)
    ...
    def update(self, tuples):
        state_batch, action_batch, TD_batch, neglogp_batch, GAE_batch = self.computeTDandGAE(
            tuples)
    ...
    def computeTDandGAE(self, tuples):
        ...
        for data in tuples:
            ...
            for i in reversed(range(len(data))):
                ...
                t = integrate.quad(lambda x: pow(
                    self.gamma, x), 0, timestep)[0]
                delta = t * rewards[i] + values[i+1] * \
                    pow(self.gamma, timestep) - values[i]
                ad_t = delta + pow(self.lambd, timestep) * \
                    pow(self.gamma, timestep) * ad_t
                advantages[i] = ad_t
        ...
        for v in tf.compat.v1.trainable_variables():
            if v.name[:-2] in saved_dict:
                ...
                if v.shape == saved_v.shape:
                    ...
                    restore_op.append(v.assign(saved_v))
                    print("Restore {}, add {} input nodes".format(
                        v.name[:-2], l))
                    ...
                    saved_v = np.concatenate((saved_v, new_v), axis=0)
                    restore_op.append(v.assign(saved_v))
                    print("Restore {}, add {} output nodes".format(
                        v.name[:-2], l))
    ...
    def printNetworkSummary(self):
        print_list = []
        print_list.append('noise : {:.3f}'.format(
            self.sess.run(self.actor.std).mean()))
        ...
        for it in range(num_iteration):
            ...
            while True:
                ...
                for j in range(self.num_slaves):
                    if not self.env.getTerminated(j):
                        if rewards[j] is not None:
                            epi_info[j].append(
                                [states[j], actions[j], rewards[j], values[j], neglogprobs[j], times[j]])
                            local_step += 1
                ...
                if local_step >= self.steps_per_iteration:
                    if self.env.getAllTerminated():
                        print('iter {} : {}/{}'.format(it+1, local_step,
                              self.steps_per_iteration), end='\r')
                        break
            ...
            if it_cur % 5 == 4:
                ...
                if self.directory is not None and self.reward_max < summary['r_per_e']:
                    ...
                    os.system("cp {}/network-{}.data-00000-of-00001 {}/network-rmax.data-00000-of-00001".format(
                        self.directory, 0, self.directory))
                    os.system(
                        "cp {}/network-{}.index {}/network-rmax.index".format(self.directory, 0, self.directory))
                    os.system(
                        "cp {}/network-{}.meta {}/network-rmax.meta".format(self.directory, 0, self.directory))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--screen", type=bool, default=False)
    ...
    env = Monitor(ref=args.ref, num_slaves=args.nslaves,
                    directory=directory, plot=args.plot, screen=args.screen)

    ppo = PPO()

    ppo.initTrain(env=env, name=args.test_name,
                  directory=directory, pretrain=args.pretrain)
